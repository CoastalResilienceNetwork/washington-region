	...
	function (declare,
		...
		var cdTool = function(container, map, app, configFile){
			...
			this.loadLayers = function(){
				...
				this.profileLandPoints = new FeatureLayer(_data[this.parameters.deltaKey].layers.profileLandPoints, {
				...
				//Profile points
				this.marshLayer = new FeatureLayer(_data[this.parameters.deltaKey].layers.marshLayer, {
				...
				//Profile points
				this.dikeLayer = new FeatureLayer(_data[this.parameters.deltaKey].layers.dikeLayer, {
...
// 					domStyle.set(sliderBar, "background-color", "rgba(148, 175, 0, 0.4)!important");
			...
			this.setSliderBounds = function(value){
				...
				if (value[0] > this.parameters.dikeCrest){
				...
				if (value[1] > this.parameters.dikeCrest){
					
					var seaEdge = this.rangeSlider.value[0];
					var shoreEdge = Math.round(this.parameters.dikeCrest, 0)-1;
					
					// dojo.connect(self.rangeSlider, "onChange", function(){
					// 	console.log(seaEdge, shoreEdge, self.rangeSlider.value);
					// 	setTimeout(function(){alert('Marsh extent limited by dike'), 1000});
					// 	dojo.disconnect(this.rangeSlider);
					// 	
					// });
			...
			this.profileResults = function(results, messages){
			    ...
			    //Wave Heights
			    self.results.waveHeightVeg_cur = eval('(' + results[1].value + ')');
			    self.results.waveHeightNoveg_cur = eval('(' + results[2].value + ')');
	    
			    self.results.waveHeightVeg_fut = eval('(' + results[3].value + ')');
			    self.results.waveHeightNoveg_fut = eval('(' + results[4].value + ')');
			    ...
			    self.results.Xveg_cur = eval('(' + results[5].value + ')');
			    self.results.Xveg_fut = eval('(' + results[6].value + ')');
		        ...
		        //Profile Height
		        self.results.ProfileHeight_cur = eval('(' + results[7].value + ')');
