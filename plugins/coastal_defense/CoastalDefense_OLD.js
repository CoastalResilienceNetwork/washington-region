	...
	function (declare,
		...
		var cdTool = function(container, map, app, configFile){
			...
			this.initialize = function(){
				...
				_data = parseConfigData(configFile);
			...
			var showIntro = function(){
					...
					_.each(_data, function(value, key){
						...
						menuItem1 = new MenuItem({
							...
							onClick: function(){showStartWindow(this.label, key)}
						});
			...
			var showStartWindow = function(delta, key){
				...
				//Load GIS Layers
				var layers = loadLayers(this);
				var extent = setExtent(this);
				...
				this.clickhandle = dojo.connect(_map,"onClick", function(evt) {processPoint(evt,self)}, this);
				
				
				
			} //end showStartWindow
			
			var processPoint = function(evt, self){
				
				dojo.disconnect(this.clickhandle);
				
				domStyle.set(this.startPanel, "display", "none");
				var showLoadingMsg = profileLoadingMsg();
				...
				gp.execute(params, closestPoint, function(error) {console.log(error)});
			
	
				
			} //end processPoint
						
			var closestPoint = function(results, messages){
				
				
				console.log(results);
				console.log(this);
				
				this.profileNum = results[0].value;
				var landX = results[1].value;
				var landY = results[2].value;
				this.profLat = results[5].value;
				this.profLon = results[4].value;
				
				this.profLat = new Number(this.profLat+'').toFixed(4);
				this.profLat = parseFloat(this.profLat);
		
				this.profLon = new Number(this.profLon+'').toFixed(4);
				this.profLon = parseFloat(this.profLon);
		
				console.log(this.profLat);
				console.log(this.profLon);
		
	
	
				//Read in Data from Profile 
				params = {"Profile" : this.profileNum};
				console.log(params);
				...
				gp.execute(params, profileData, function(error) {console.log(error)});
				
			}
			
			var profileData = function(results, messages){
				
				
				//console.log(this);
				this.shoreDist = eval('(' + results[0].value + ')'); // * thing.unitConversion;
				this.profileElevation = eval('(' + results[1].value + ')'); // * thing.unitConversion;
				...
				this.marshSeaEdge = eval('(' + results[2].value + ')');
				this.marshLandEdge = eval('(' + results[3].value + ')');		
				this.eelSeaEdge = eval('(' + results[4].value + ')') ;
				this.eelLandEdge = eval('(' + results[5].value + ')') ;
				this.dikeCrest = eval('(' + results[6].value + ')') ;
				this.dikeHeight = eval('(' + results[7].value + ')') ;
				this.dikeAngle = eval('(' + results[8].value + ')');
				this.toeHeight = eval('(' + results[9].value + ')') ;
			...
			var profileLoadingMsg = function(){
				...
				dom.byId(_container).appendChild(profileMsgDiv);
				
			}
			
			var profileProcessingMsg = function(){
				...
				dom.byId(_container).appendChild(processMsgDiv);
			...
			var startTool = function(){
				...
				this.helpPanel = new ContentPane({
					...
					content: _data[deltaKey].help.help 
				});
				...
				//Create Sub-Panels
				var wavePanel = createWavePanel(this);
				var waterPanel = createWaterPanel(this);
				var habitatPanel = createHabitatPanel(this);
				var structurePanel = createStructurePanel(this);
				var sliderHelp = createSliderHelp(this); 
				var plotPanel = createProfilePlot(this.mainPanel);
				var sliders = addSliders(this);
				var marshTextListers = addMarshTextListeners(this);
				var buttons = addButtonsToInputPanel(this);
				var layerPanel = addLayerPanel(this);
				var toolTips = createTooltips(this);
				var helpText = setupHelp(this);
				//parser.parse();
			...
			var createWavePanel = function(){
				...
				_.each(_data[deltaKey].winds, function(value, key){
					menuItem1 = new MenuItem({
						label: value.name,
			...
			var updateWaveBtn = function(label){
			...
			var createWaterPanel = function(){
				...
				_.each(_data[deltaKey].waterLevel, function(value, key){
				...
				_.each(_data[deltaKey].stormSurge, function(value, key){
					var menuItem1 = new MenuItem({
						label: value.name,
						onClick: function(){updateSurgeBtn(this.label, key)}
					});
			...
			var updateWaterBtn = function(label, key){
			...
			var updateSurgeBtn = function(label, key){
			...
			var createHabitatPanel = function(){
				...
				_.each(_data[deltaKey].marshDensity, function(value, key){
					var menuItem1 = new MenuItem({
						label: value.name,
				...
				console.log(_data[deltaKey].marshDensity[this.marshDensityKey].value);

				
				addCheckboxDisableListener();
				
				
			} //End createHabitatPanel
						
			var updateMarshDensityBtn = function(label, key){
			...
			var createStructurePanel = function(){
			...
			var createProfilePlot = function(){
			...
			var positionProfileLegend = function(){
			...
			var addSliders = function() {
				...
				addCheckboxListerForSliders();
				
				
			}
			
			var setSliderBounds = function(value){
				...
				var dist_0 = number.round((value[0]/_data[deltaKey].profilePointSpacing),0) * _data[deltaKey].profilePointSpacing;
				var dist_1 = number.round((value[1]/_data[deltaKey].profilePointSpacing), 0) * _data[deltaKey].profilePointSpacing;
				...
				var marshmin = Math.min(_data[deltaKey].marshBoundary[0], this.marshSeaEdge)  
				var marshmax = Math.max(_data[deltaKey].marshBoundary[1], this.marshLandEdge)
			...
			var addMarshTextListeners = function(){
			...
			var createTooltips = function(){
				...
				this.marshHighTooltip = new dijit.TooltipDialog({
					id: 'marshHighTooltip',
					//style: "width: 300px;",
					content: "Marsh must be below" + _data[deltaKey].marshBoundary[1] + " above MSL."
				});
				
				this.marshLowTooltip = new dijit.TooltipDialog({
					id: 'marshLowTooltip',
					//style: "width: 300px;",
					content: "Marsh must be above " + _data[deltaKey].marshBoundary[0] + " MSL.",
				});
			...
			var setupHelp = function(){
				...
				dojo.connect(dijit.byId("waveRadio1"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.windWave)
				});
				
				dojo.connect(dijit.byId("waveHeight"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.windWave)
				});
				
				dojo.connect(dijit.byId("wavePeriod"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.windWave)
				});
				
				dojo.connect(dijit.byId("windButton"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.windWave)
				});
				
				dojo.connect(dijit.byId("waterButton"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.waterLevel)
				});
				
				dojo.connect(dijit.byId("surgeButton"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.stormSurge)
				});
				
				dojo.connect(dijit.byId("cd_marsh"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.habitat)
				});
				
				dojo.connect(dijit.byId("cd_shoreEdgeBox"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.habitat)
				});
				
				dojo.connect(dijit.byId("cd_seaEdgeBox"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.habitat)
				});
				
				dojo.connect(dijit.byId("cd_marshDensityButton"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.habitatDensity)
				});
				
				dojo.connect(dijit.byId("cd_dike"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.dikes)
				});
				
				dojo.connect(dijit.byId("cd_slopeBox"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.dikeSlope)
				});
				
				dojo.connect(dijit.byId("cd_heightBox"), "onClick", function(){
					self.helpPanel.set('content', _data[deltaKey].help.dikeHeight)
				});
				
			}
			
			var addLayerPanel = function(){
			...
			var addCheckboxListerForSliders = function() {
			...
			var addCheckboxDisableListener = function(){
			...
			var addButtonsToInputPanel = function(){
			    ...
			    var scenarioButton = new Button({
			        label: "Run Scenario",
					id: "cd_scenarioBtn",
			        onClick: function(){runScenario(this)}
			    });
				
			    var newProfileButton = new Button({
			        label: "Select New Profile",
					id: "cd_newProfileBtn",
			        onClick: function(){newProfileButtonHandler(self)}
			    });
			...
			var runScenario = function(){
				
				domStyle.set(this.cd_borderContainer.domNode, "display", "none");
				var showProcessingMsg = profileProcessingMsg();
				...
				var wl = _data[deltaKey].waterLevel[this.waterLevelKey].value;	
				var surge = _data[deltaKey].stormSurge[this.surgeLevelKey].value;
				...
				var marshDensity = _data[deltaKey].marshDensity[this.marshDensityKey].value;
				...
				gp.execute(params, profileResults, function(error) {
			...
			var profileResults = function(results, messages){
				...
				this.detailedResults = results[0].value;
				
			    //Wave Heights
			    this.waveHeightVeg_cur = eval('(' + results[1].value + ')');
			    this.waveHeightNoveg_cur = eval('(' + results[2].value + ')');
	    
			    this.waveHeightVeg_fut = eval('(' + results[3].value + ')');
			    this.waveHeightNoveg_fut = eval('(' + results[4].value + ')');


			    //X Axis - distance from MSL
			    this.Xveg_cur = eval('(' + results[5].value + ')');
			    this.Xveg_fut = eval('(' + results[6].value + ')');

		        //Profile Height
		        this.ProfileHeight_cur = eval('(' + results[7].value + ')');
				
		        // Overtopping Output
		        this.dikemsg = results[8].value;
		        this.dikemsgMA = results[9].value;
				this.dikemsgNo = results[10].value;
				
				
				console.log(this.waveHeightVeg_cur);
				console.log(this.waveHeightNoveg_cur);
				console.log(this.waveHeightVeg_fut);
				console.log(this.waveHeightNoveg_fut);
				console.log(this.Xveg_cur);
				console.log(this.Xveg_fut);
				console.log(this.ProfileHeight_cur);
				console.log(this.dikemsg);
				console.log(this.dikemsgMA);
				
				var resultsPanel = createResultsPanel(this);
			...
			var plotResults = function(){
				 ...
				 _.each(this.Xveg_cur, function(value, key){
					 ...
					 var item = {
						 ...
						 x: value,
						 y: this.waveHeightNoveg_cur[key]
					 }
					 ...
					 var item = {
						 ...
						 x: value,
						 y: this.waveHeightNoveg_fut[key]
					 }
				 
					 waveHeightNoveg_fut_plotting.push(item);	
					 
					 //Current Change Due to Habitat
					 var tmp = this.waveHeightVeg_cur[key] / this.waveHeightNoveg_cur[key] * 100;
					 ...
					 var tmp = this.waveHeightVeg_fut[key] / this.waveHeightNoveg_fut[key] * 100;
					 var item = {
				...
				waveHeightChart.addPlot("waveHeight", {type: Lines});
				waveHeightChart.addPlot("marsh", {type: Areas});
				waveHeightChart.addAxis("x", {title: xTitle, font: axisFonts, titleOrientation: 'away', titleGap: 10 });
				waveHeightChart.addAxis("y", {vertical: true, title: yTitle, font: axisFonts});   //, min: -2, max: 2});
							
				
				waveHeightChart.addSeries("Current Scenario", waveHeightVeg_cur_plotting,
					{plot: "waveHeight", tension:"S", stroke: {color:"#1589FF", width: 3}});  
					
				waveHeightChart.addSeries("Future Scenario", waveHeightVeg_fut_plotting,
					{plot: "waveHeight", tension:"S", stroke: {color:"#ffd700", width: 3}}); 
					
				waveHeightChart.addSeries("No Vegetation", waveHeightNoveg_cur_plotting,
					{plot: "waveHeight", tension:"S", stroke: {color:"#000000", width: 3}}); 
					
				
				waveHeightChart.render();
				
				//Get y axis min and max
				var ymin = this.waveHeightChart.axes.y.getTicks().major[0].value;
				var ymax = Math.max(_.last(this.waveHeightChart.axes.y.getTicks().major).value, _.last(this.waveHeightChart.axes.y.getTicks().minor).value );
				...
				if(this.dikeCheck == true){
				    waveHeightChart.addSeries("Dike Position", dikePosition,
				...
				if(this.marshCheck == true){
				    waveHeightChart.addSeries("Current Marsh Extent", marshExtent,
						{plot: "marsh", stroke: {color:"rgba(0,127,0,0)"}, fill: "rgba(0,127,0,0.3)"});
				}
				
				if(this.marshCheck_Fut == true){
				    waveHeightChart.addSeries("Future Marsh Extent", marshExtent_Fut,
				...
				var legend = new Legend({ id: "cd_waveHeightLegend", class: "cd_waveHeightLegend", chart: waveHeightChart, layoutAlign: "Right"}, "cd_waveHeight_legendDiv");
				...
				waveChangeChart.addPlot("waveChange", {type: Lines});
				waveChangeChart.addPlot("marshChange", {type: Areas});
				waveChangeChart.addAxis("x", {title: xTitle, font: axisFonts, titleOrientation: 'away', titleGap: 10 });
				waveChangeChart.addAxis("y", {vertical: true, title: yTitle, font: axisFonts});   //, min: -2, max: 2});
				
				
				waveChangeChart.addSeries("Current Scenario", HChange_cur,
					{plot: "waveChange", tension:"S", stroke: {color:"#1589FF", width: 3}});  
					
				waveChangeChart.addSeries("Future Scenario", HChange_fut,
					{plot: "waveChange", tension:"S", stroke: {color:"#ffd700", width: 3}}); 
				
				
				waveChangeChart.render();
				...
				if(this.dikeCheck == true){
				    waveChangeChart.addSeries("Dike Position", dikePosition_ChangePlot,
				...
				if(this.marshCheck == true){
				    waveChangeChart.addSeries("Current Marsh Extent", marshExtent_ChangePlot,
						{plot: "marshChange", stroke: {color:"rgba(0,127,0,0)"}, fill: "rgba(0,127,0,0.3)"});
				}
			
				if(this.marshCheck_Fut == true){
				    waveChangeChart.addSeries("Future Marsh Extent", marshExtent_Fut_ChangePlot,
						{plot: "marshChange", stroke: {color:"rgba(0,127,0,0)"}, fill: "rgba(0,127,0,1)"});
				}
				
				waveChangeChart.render();
				
				var legend = new Legend({ id: "cd_waveChangeLegend", class: "cd_waveHeightLegend", chart: waveChangeChart, layoutAlign: "Right"}, "cd_waveChange_legendDiv");
			...
			var addButtonsToResultsPanel = function(){
			    ...
			    var differentScenarioButton = new Button({
			        ...
			        onClick: function(){differentScenarioBtnHandler(self)}
			    });
			...
			var differentScenarioBtnHandler = function(){
			...
			var newProfileButtonHandler = function(){
			...
			var parseConfigData = function(configFile) {
